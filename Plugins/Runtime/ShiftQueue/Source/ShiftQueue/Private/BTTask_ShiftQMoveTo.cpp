...
void UBTTask_ShiftQMoveTo::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
	if (bHasTaskFinished)
	{
		FinishLatentTask(OwnerComp, TaskNodeResult);
		bHasTaskFinished = false;
	}
}
...
{
	bHasTaskFinished = true;
	if (PathFollowingResult.IsFailure())
	{
		TaskNodeResult = EBTNodeResult::Failed;
	}
	else if (PathFollowingResult.IsInterrupted())
	{
		TaskNodeResult = EBTNodeResult::Aborted;
	}
	else if (PathFollowingResult.IsSuccess())
	{
